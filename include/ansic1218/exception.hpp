...
class Exception : public std::runtime_error {
    ...
    template <typename Arg, typename... Args> void push(std::ostringstream &out, Arg &&arg, Args &&...args)
    {
        out << arg;
        push(out, args...);
    }

    template <typename Arg, typename... Args> std::string build(Arg &&arg, Args &&...args)
    {
