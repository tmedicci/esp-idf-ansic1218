...
TEST_CASE("Should fail to request partial read because offset is higher than allowed (24 bits)", "[ansic1218][services][ReadPartial]")
{
...
TEST_CASE("Should not fail to request partial read because offset is equal than max allowed value (24 bits)", "[ansic1218][services][ReadPartial]")
{
...
TEST_CASE("Should request and validate half of a big table properly", "[ansic1218][services][ReadPartial]")
{
    ...
    uint8_t checksum = 0;
    for_each(table_data.begin() + 500, table_data.end(), [&checksum](uint8_t value)
             { checksum += value; });
    checksum = ~checksum;
